...
class Datasource {
    ...
    public static function getRedis($config_name = "instance1", $server_region = 'default') {
        if ($config_name === NULL) {
            return;
        }
    ...
    public static function getCache($config_name = "instance1", $server_region = 'default') {
        ...
        $config=Conf::get("memcache");    //获取配置
        $memcache_config = $config[$config_name];
        try {
...
class RedisHelper {
    ...
    public function __call($name, $arguments) {
        ...
        DebugLog::_redis($name, $arguments, $redis_config, $starttime, microtime(), (is_string($data) || is_array($data)) ? $data : null);
        return $data;
    }
...
class CacheHelper {
    ...
    private function __construct($config_name = NULL, $memcache_config = NULL, $server_region = "default")
    {
        if ($config_name === NULL) {
            return;
        }
		...
		// var_dump($this->memcache_config);
        foreach ($this->memcache_config as $server) {
            // $timeout = $server['timeout'] ? $server['timeout'] : $this->timeout;
            $this->_memcache->addServer($server['host'], $server['port'], true);
        }
        $mtime2 = microtime();
        DebugLog::_cache('connect', null, $memcache_config, $mtime1, $mtime2, null);
    }
    ...
    public function __call($name, $arguments) {
        ...
        $mtime2 = microtime();
        DebugLog::_cache('call:' . $name, $arguments, $this->memcache_config, $mtime1, $mtime2, $data);
        return $data;
    }
    function __destruct()
    {
